...
const MovieDetails = ({ navigation, route }) => {
  ...
  const getInfosDetail = ({
    ...
    Durée: convertMinsToHrsMins(runtime),
    Genre: convertToGenres(sliceArrayLength(genres, 2)),
    Langage: convertToUpperCaseFirstLetter(isoLanguage[original_language]),
    Sortie: convertToDate(release_date),
    Budget: convertToDolar(budget),
    Revenu: convertToDolar(revenue),
    Adulte: ADULT_RATE[adult] || UNINFORMED
  });
          ...
          {isLoading ? (
            ...
            <ScrollView>
              ...
              <View style={styles.containerMovieInfo}>
                ...
                <SectionRow title="Résumé">
                  <ReadMore
                    numberOfLines={3}
                    renderTruncatedFooter={(handlePress) =>
                      renderReadMoreFooter('Lire plus', handlePress)
                    }
                    renderRevealedFooter={(handlePress) =>
                      renderReadMoreFooter('Lire moins', handlePress)
                    }
                  >
                    <Text style={styles.subTitleInfo}>{overview}</Text>
                  </ReadMore>
                </SectionRow>
                <SectionRow title="Rôle principal">
                  <PersonListRow
                    data={cast}
                    type="character"
                    keyItem="creditId"
                ...
                <SectionRow title="Equipe technique principale">
                  <PersonListRow
                    data={crew}
                    type="job"
                    keyItem="creditId"
                ...
                <SectionRow title="Producteurs" isLast>
                  <PersonListRow
